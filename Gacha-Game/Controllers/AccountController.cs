...
namespace Gacha_Game.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        /// <returns>
        /// An <see cref="ActionResult{T}"/> containing an <see cref="ApiResponse"/> indicating the success or failure of the operation.
        /// </returns>
        /// <response code="200">Currency increased successfully.</response>
        /// <response code="404">User not found.</response>
        /// <response code="500">Internal server error.</response>
        [HttpPost("increase-currancy")]
        public async Task<ActionResult<ApiResponse>> IncreaseCoin(IncreaseCurrancyDto currancyDto)
        {
            _logger.LogInformation("Increasing coin for userID: {UserID} by amount: {Amount}", currancyDto.UserID, currancyDto.Amount);

            var user = await _userManager.FindByIdAsync(currancyDto.UserID.ToString());
            if (user == null)
            {
                _logger.LogWarning("User not found for userID: {UserID}", currancyDto.UserID);
                return NotFound(new ApiResponse
                {
                    Message = "User not found",
                    StatusCode = System.Net.HttpStatusCode.NotFound,
            ...
            await _authenticationServices.IncreaseCurrancy(currancyDto);
            await _unitOfWork.CompleteAsync();

            _logger.LogInformation("Coin increased successfully for userID: {UserID}", currancyDto.UserID);
            return Ok(new ApiResponse
            {
